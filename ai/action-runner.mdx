...
<AccordionGroup>
  ...
  <Accordion title="stagehand_cookie_login — authenticate using stored cookies">
    Decrypts the stored AES-256-GCM credentials for a creator profile, injects the cookies into a new browser session, and navigates to the target platform URL. Returns `{ sessionId, navigated: true }`.
    ...
    ```json
    {
      ...
      "result_key": "session",
      "config": {
        "creator_profile_id": "$input.creator_profile_id",
        "platform": "onlyfans",
        "target_url": "https://onlyfans.com/my/statistics"
      }
    }
    ```

    Used in scrape flows instead of `stagehand_start` + `stagehand_navigate` when authenticated sessions are required.
  </Accordion>

  <Accordion title="stagehand_extract_cached — extract with cache check">
  ...
  <Accordion title="ollama_chat — multi-turn conversation">
  ...
  <Accordion title="taxonomy_terms_read — cached taxonomy lookup">
  ...
  <Accordion title="taxonomy_platform_maps_read — cached platform taxonomy maps">
  ...
  <Accordion title="taxonomy_invalidate — bust the taxonomy cache">
