...
|---|---|
| `directus_read` | Read items from a Directus collection (row-isolated to `req.directusUser`) |
| `directus_write` | Create or update items in a Directus collection via MCP |
| `directus_delete` | Delete items from a Directus collection via MCP |
| `stagehand_act` | Issue a Playwright action via the Stagehand server |
| `stagehand_extract` | Extract structured data from a browser session |
| `stagehand_observe` | Observe and describe the current browser DOM state |
| `ollama_generate` | Call the local Ollama LLM for a text generation sub-task |
| `ollama_chat` | Multi-turn chat call to the local Ollama LLM |
| `bullmq_enqueue` | Push a job onto a BullMQ queue; returns `jobId` |
| `bullmq_wait` | Poll a BullMQ job until completion or timeout |
| `mcp_tool` | Call any registered MCP tool by namespace and name |
| `transform` | Apply a data transformation (JSONPath, template substitution) |
